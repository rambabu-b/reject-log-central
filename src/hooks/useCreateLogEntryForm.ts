...
export const useCreateLogEntryForm = (onCreate: (entry: LogEntry) => void) => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      console.log(
        "No products found in localStorage, initializing sample products"
      );
    ...
    if (user?.role === "production") {
      ...
      if (!formData.polyBagNo || !formData.grossWeight) {
        toast({
          title: "Error",
          description:
            "Poly Bag No and Gross Weight are required for production entries",
          variant: "destructive",
        });
        return false;
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newEntry: LogEntry = {
      ...
      createdAt: new Date().toISOString(),
      assignedProductionUser:
        formData.assignedProductionUser ||
        (user?.role === "production" ? user.id : ""),
      ...
      productionTimestamp: formData.productionConfirmed
    ...
    toast({
      title: "Success",
      description: `Rejection log entry created successfully and assigned to ${
        initialStatus === "stores_pending" ? "stores team" : "production team"
      }`,
    });
  };
